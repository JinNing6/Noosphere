...
class VerifyPypiReleaseTests(unittest.TestCase):
    def test_validate_release_json_requires_matching_version_and_artifacts(self):
        data = {
            "info": {"version": "0.6.7"},
            "urls": [
                {"filename": "noosphere_mcp-0.6.7-py3-none-any.whl"},
                {"filename": "noosphere_mcp-0.6.7.tar.gz"},
            ],
        }

        filenames = validate_release_json(data, "0.6.7")

        self.assertEqual(
            filenames,
            ["noosphere_mcp-0.6.7-py3-none-any.whl", "noosphere_mcp-0.6.7.tar.gz"],
        )

    def test_validate_release_json_rejects_missing_wheel(self):
        data = {
            "info": {"version": "0.6.7"},
            "urls": [{"filename": "noosphere_mcp-0.6.7.tar.gz"}],
        }

        with self.assertRaisesRegex(RuntimeError, "missing distribution"):
            validate_release_json(data, "0.6.7")
    ...
    def test_inspect_installed_release_requires_version_tool_count_and_growth_tools(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            package_root.mkdir()
            (package_root / "__init__.py").write_text('__version__ = "0.6.7"\n', encoding="utf-8")
            ...
            result = inspect_installed_release(target, "0.6.7", expected_tool_count=40)
    ...
    def test_install_release_to_target_uses_exact_version_and_no_deps(self):
        with patch("scripts.verify_pypi_release.subprocess.run") as run:
            install_release_to_target("noosphere-mcp", "0.6.7", Path("target"), python_executable="python")

        command = run.call_args.args[0]
        self.assertIn("--no-deps", command)
        self.assertIn("--target", command)
        self.assertIn("noosphere-mcp==0.6.7", command)
    ...
    def test_wait_for_pypi_project_latest_requires_latest_version(self):
        ...
        with (
            ...
            result = wait_for_pypi_project_latest("noosphere-mcp", "0.6.7", attempts=2, delay_seconds=0)

        self.assertEqual(result["info"]["version"], "0.6.7")
    ...
    def test_wait_for_installable_release_retries_pip_index_lag(self):
        with (
            ...
            wait_for_installable_release("noosphere-mcp", "0.6.7", Path("target"), attempts=2, delay_seconds=0)
    ...
    def test_verify_pypi_release_waits_installs_and_inspects(self):
        ...
        installed = {"version": "0.6.7", "tool_count": 40, "growth_tools": REQUIRED_GROWTH_TOOLS}
        ...
        with (
            ...
            result = verify_pypi_release("noosphere-mcp", "0.6.7", attempts=1, delay_seconds=0, expected_tool_count=40)

        wait.assert_called_once_with("noosphere-mcp", "0.6.7", 1, 0)
        wait_latest.assert_called_once_with("noosphere-mcp", "0.6.7", 1, 0)
        install.assert_called_once_with("noosphere-mcp", "0.6.7", ANY, 1, 0)

...
async def test_launch_preflight_reports_release_and_install_blockers(mock_env):
    ...
    respx.get("https://api.github.com/repos/test_owner/test_repo/git/ref/tags/v0.6.4").mock(
        return_value=Response(404, json={"message": "Not Found"})
    )
    respx.get("https://api.github.com/repos/test_owner/test_repo/releases/tags/v0.6.4").mock(
        return_value=Response(404, json={"message": "Not Found"})
    )
    respx.get("https://jinning6.github.io/Noosphere/traction_proof.json").mock(
        return_value=Response(200, json={
            ...
            "bottleneck": {
                ...
                "reason": "PyPI latest 0.6.0 does not match local package 0.6.4.",
            },
            "first_proof_action": {
    ...
    result = await launch_preflight(target_version="0.6.4")

    assert "Noosphere launch preflight" in result
    assert "Target package: noosphere-mcp==0.6.4" in result
    assert "PyPI latest: 0.6.0" in result
    assert "Release tag v0.6.4: missing" in result
    assert "Trusted Publishing trigger: tag-or-release" in result
    assert "GitHub Release v0.6.4: missing" in result
...
async def test_launch_preflight_reports_ready_when_release_and_registry_match(mock_env):
    respx.get("https://pypi.org/pypi/noosphere-mcp/json").mock(
        return_value=Response(200, json={"info": {"version": "0.6.4"}})
    )
    respx.get("https://api.github.com/repos/test_owner/test_repo/git/ref/heads/main").mock(
...
async def test_launch_preflight_accepts_annotated_tag_without_github_release(mock_env):
    ...
    assert "Status: ready" in result
    assert "Release tag v0.6.4: aligned" in result
    assert "Trusted Publishing trigger: tag-or-release (available)" in result
    assert "GitHub Release v0.6.4: missing (optional; v* tag trigger is active)" in result

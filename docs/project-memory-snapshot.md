...
- PyPI still reports `noosphere-mcp==0.9.2` as latest and the official `0.10.0` version endpoint returns HTTP 404. No `v0.10.0` tag, GitHub Release, Trusted Publishing run, public artifact, or deployment is claimed. The next boundary is PR CI; formal publication remains a separate release action after review and merge.

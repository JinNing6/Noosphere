...
- Final recovery still requires merging PR #47 and a Glama admin deployment using the repository-root Dockerfile. Completion requires the public Glama API to expose 45 tools, optional authentication, and current metadata; a GitHub release sync alone is not sufficient.

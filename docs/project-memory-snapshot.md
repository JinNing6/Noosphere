...
## Experience Protocol v0.1 — Fully Automated And Deployed

- PR [#90](https://github.com/JinNing6/Noosphere/pull/90) merged into `main` as `1aac8094bfb72d21be14e752115eed5e046d6ef8` on 2026-07-31. It deploys the experimental first-class Experience Record and its zero-service GitHub Experience Agent. Experience, Evidence, Skill, and Outcome remain separate objects: Experience is descriptive data and never becomes executable authority merely by being accepted.
- The public lifecycle is fully automated for Experience submissions: a consent-bearing Issue binds the authenticated author and stable record identity; exact declared public workflow provenance, schema, privacy, safety, reference, redaction, and repository gates run; a passing record receives an explicit `automated-policy` approval, is committed directly to `main`, receives exact success labels and one idempotent receipt, and the Issue closes as completed. No maintainer review or merge action remains in this path.
- Failed submissions remain open with exact findings. An invalid edit to an accepted Issue reopens it, removes stale success labels, preserves the last passing canonical record, and retries the same authenticated identity and stable path after correction. All canonical writers use `noosphere-main-writer`; the workflow rejects a stale `main`, stages only the derived Experience paths, and reruns canonical tests itself because `GITHUB_TOKEN` writes do not recursively trigger ordinary push workflows.
- Automated acceptance is repository policy approval for non-executable descriptive data, not human review, independent reproduction, immutable Skill publication, or callability. `review.mode` distinguishes `automated-policy` from `human`; an automated reviewer and timestamp must match the recognized screening receipt. Experience-to-Skill promotion remains a separate reviewed supply chain.
- The first record, `exp-codex-session-junction-migration-20260731`, is now under `experience_records/reviewed/` with matching `repository-policy-gate-v1` screening and automated-review receipts. It is a redacted, `locally-verified` projection of the completed Windows migration, not independently reproduced. This bootstrap receipt is intentional because the Issue workflow did not exist on the default branch when the record was prepared; it is not falsely attributed to `github-experience-agent-v1`.
- The implementation uses public GitHub-hosted Actions and the repository token only: no paid API, model call, database, MCP tool, always-on service, or submitted-code execution. The workflow checks out trusted `main`, pins third-party Actions to exact commits, and verifies declared workflow evidence against the exact public repository, full commit SHA, run, job, and step.
- Local verification passed 11 focused Experience Agent tests, 30 Experience protocol tests, all 91 repository Python tests, and all 133 Node workflow/supply-chain tests; Ruff `0.14.1`; workflow YAML and Action static validation; and `jsonschema==4.26.0` Draft 2020-12 validation. The strict remote-facing hygiene scan passed 473 files, and changed-file credential scanning found no credential-shaped or sensitive-field material. GitHub run [30618595879](https://github.com/JinNing6/Noosphere/actions/runs/30618595879) passed every applicable Linux, Windows, macOS, Python, Experience, Shared Skill, CLA, and routing gate before merge. The workflow is deployed; a first post-deployment external Experience Issue remains the next production observation rather than a prerequisite for availability.

...
- The only remaining external step is Glama admin `Sync Server` or its next daily sync. Completion requires the public Glama API to expose 45 tools, optional authentication, and current metadata rather than merely accepting the repository configuration; immediately after release it still returned the stale empty-tool snapshot.

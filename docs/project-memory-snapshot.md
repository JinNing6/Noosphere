...
- Final recovery still requires a successful Linux container CI run, merge, and a Glama admin deployment using the repository-root Dockerfile. Completion requires the public Glama API to expose 45 tools, optional authentication, and current metadata; a GitHub release sync alone is not sufficient.

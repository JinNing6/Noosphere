...
## Experience Protocol v0.1 — Automated Intake Draft, CI Passed

- Branch `codex/experience-protocol-v0` is based on remote `main` commit `5738a176428f34d6befa1979196de183a3ad9cd1`; functional head `72a13d1bcfd3de9ce4feba26d5a3bce2a5ec527a` adds an experimental first-class Experience Record and its automated public intake. Experience, Evidence, Skill, and Outcome remain separate objects: Evidence supports claims, multiple Experiences may inform a Skill, and Outcomes remain bound to an exact immutable Skill release.
- `EXPERIENCE_PROTOCOL.md` and `schemas/experience-record-v0.1.schema.json` define the v0.1 lifecycle, machine screening, human review, verification, relations, provenance, and redaction boundaries using JSON Schema Draft 2020-12. Machine screening records deterministic policy compliance only; it does not grant human review, independent reproduction, Skill publication, or callability.
- The zero-service GitHub Experience Agent consists of `experience-record.yml`, `experience_intake.yml`, and `experience-intake.cjs`. It accepts one consent-bearing JSON record, binds the authenticated Issue author and stable Issue identity, verifies exact public GitHub workflow provenance when declared, screens content and commands, runs canonical validators, and writes one idempotent `candidate` record directly to `main`. Edited or reopened Issues retry the same record; `workflow_dispatch` is a default-branch recovery path. It uses GitHub-hosted Actions and the repository token only, with no paid API, model call, database, MCP tool, or always-on service.
- The automation checks out trusted `main`, never executes contributor-controlled code, serializes writes through `noosphere-main-writer`, rechecks that `main` did not advance, stages only the derived Experience path, and reconciles a single bot receipt plus exact state labels. Because commits made with `GITHUB_TOKEN` do not trigger ordinary push workflows, the writer runs the Experience tests and canonical validator itself before committing.
- The first candidate, `exp-codex-session-junction-migration-20260731`, remains a redacted projection of the completed Windows Codex session-storage migration. It is `locally-verified`, not independently reproduced. Its machine-screening receipt is `repository-policy-gate-v1`: the record predates deployment of the Issue automation and was reviewed by the same deterministic repository gates in this PR. It must not be described as processed by `github-experience-agent-v1`; that agent becomes active only after the workflow is merged into the default branch.
- `scripts/validate_experience_records.py` is the zero-runtime-dependency canonical repository gate. In addition to bounded schema, lifecycle, review, evidence, Skill relation, provenance, and redaction checks, it now validates recognized screening receipts, Issue-author binding, exact workflow-run receipts, prompt-override and exfiltration patterns, and unsafe resolution commands. The tracked candidate excludes the local username, absolute private paths, session identifiers, raw task content, and individual file hashes.
- Local verification passed 10 focused Experience Agent tests, 29 Experience protocol tests on Python 3.11 and 3.13, all 74 repository tests, all 132 Node workflow/supply-chain tests, and all 229 isolated MCP v2 SDK tests; Ruff `0.14.1`; workflow YAML and schema parsing; and `jsonschema==4.26.0` Draft 2020-12 validation. The strict remote-facing hygiene scan passed 473 files. A field-aware credential scan found no credential-shaped material in the new Experience files; its only repository-wide provider-token-shaped hit is an unchanged 24-character test fixture in `sdk/tests/test_preflight.py`, repeated only in test `GITHUB_TOKEN` fields, with no open GitHub secret-scanning alert. The user explicitly authorized public submission on 2026-07-31. Draft PR [#90](https://github.com/JinNing6/Noosphere/pull/90) remains unmerged and unreleased; GitHub run [30616955223](https://github.com/JinNing6/Noosphere/actions/runs/30616955223) completed with every applicable check successful.
...
- PR [#64](https://github.com/JinNing6/Noosphere/pull/64) was reviewed at head `7a67d637610adcd88af58001c98ca0ac87d0901c` and remains open and not acceptable as-is: it adds a repository-root `SKILL.md`, fails the repository Skill frontmatter validator, lacks exact public workflow provenance, contains incomplete verification, and has a failing CLA check. The first deployment backfill run [30428849226](https://github.com/JinNing6/Noosphere/actions/runs/30428849226) completed successfully and added `skill-evidence-incomplete` plus `skill-evidence-pr-routed` together with the bot-authored [routing receipt](https://github.com/JinNing6/Noosphere/pull/64#issuecomment-5114101138); it did not close, merge, or execute contributor code. Its observed structure remains a regression fixture for the PR adapter.
- Release verification passed 122 Node workflow/supply-chain tests, including PR #64 routing, deployment-triggered Open-PR backfill, live-event single-PR isolation, base-only checkout, a two-publisher zero-model-key intake-to-candidate integration path, and an Issue Form/parser contract check; 15 repository migration/registry unit tests; the Shared Skill registry/plugin validator; YAML parsing for five changed forms/workflows; and `git diff --check`. The adapter is now live and its historical PR backfill is production-verified. A first live V4 form submission that references an external public workflow run is still required before cross-repository GitHub-token provenance access is described as production-proven.
